...
export async function appendTrainingCapture(entry, { storageKey, maxEntries } = {}) {
	try {
	...
	try {
		...
		if (_lsSet(key, raw)) return { ok: true, backend: "localStorage", n: sliced.length };

		const shrunk = sliced.slice(Math.max(0, sliced.length - Math.max(50, Math.floor(limit / 2))));
		if (_lsSet(key, JSON.stringify(shrunk))) return { ok: true, backend: "localStorage", n: shrunk.length, truncated: true };
