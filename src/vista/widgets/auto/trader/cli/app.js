...
function usage() {
  return [
    ...
    "  --run-profile            Runs the real auto-bot headlessly (no UI) using a named profile.",
...
async function runProfile(argv) {
  ...
  // Import real bot module.
  const mod = await import("../index.js");

  // Apply profile + start.
  mod.__fdvDebug_setOverrides({ ...(globalThis.__fdvAutoBotOverrides || {}), headless: true });
  mod.__fdvCli_applyProfile(profile);

  console.log(`Running profile '${String(profileName)}' from ${profilesPathOrUrl}`);
  console.log("Press Ctrl+C to stop.");

  const ok = await mod.__fdvCli_start({ enable: true });
  if (!ok) {
    console.error("Headless start failed (see logs above).");
    return 3;
  }
  ...
  const stop = async () => {
    try {
      console.log("\nStopping…");
      await mod.__fdvCli_stop({ runFinalSellEval: true });
    } finally {
      process.exit(0);
    }
  };
